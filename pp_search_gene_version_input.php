<?php
	function getCheckboxes($chkId, $chkName)
	{
...
?>
	...
	<?php
	echo "<input type=\"checkbox\" id=\"{$chkId}\" class=\"checkbox\" name=\"version_search\">Search for a specific gene version</input>";
	?>
	...
	<?php
		echo "$('#{$chkId}').change(function()";
		?>
		{ 
			if($(this).is(":checked")) {
				<?php
			echo " $('[name=\"{$chkName}[]\"]').prop('disabled',false);\n}";
			echo "else \n { \n";
			echo "$('[name=\"{$chkName}[]\"]').prop('disabled',true);\n}";
		?>		
		});
	</script>
	<div class="form-check-inline">
		<?php
			...
			{
				echo "<input type='checkbox' disabled='true' name='{$chkName}[]' value='{$version}' class='form-check-input checkbox'>" . $version . "</input>";
			}
		?>
	</div>
</div>
